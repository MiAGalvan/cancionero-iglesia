// Trabajo automático (ver vercel.json → crons) que corre solo, una vez por
// día de madrugada: trae el Evangelio, la 1ª Lectura, el Salmo y la 2ª
// Lectura (cuando hay) del feed público de evangelizo.org, y los publica
// en Novedades para TODAS las parroquias — el calendario litúrgico es el
// mismo en cualquier lado, así que no hace falta cargarlo a mano en cada
// una.
//
...
// este corrimiento simple, no hace falta Intl con timezone.
function hoyArgentinaIso() {
  const ahoraArg = new Date(Date.now() - 3 * 60 * 60 * 1000);
  return ahoraArg.toISOString().slice(0, 10);
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const itemsDeHoy = items.filter((item) => item.fecha === hoy && CATEGORIA_A_TITULO[item.category]);
    if (itemsDeHoy.length === 0) {
      await registrarEstado({ exito: false, error: `El feed todavía no tiene las lecturas de hoy (${hoy})`, espaciosActualizados: 0 });
      res.status(200).json({ ok: false, motivo: 'sin-lecturas-de-hoy-todavia', hoy });
      return;
    }
    ...
    for (const espacio of espacios) {
      ...
      for (const item of itemsDeHoy) {
        ...
        if (existente && existente.fecha === hoy && existente.auto_generated === false) continue;
    ...
    res.status(200).json({ ok: true, hoy, espacios: espacios.length, filasEscritas });
  } catch (error) {
